...
CREATE OR REPLACE PACKAGE BODY search_movies AS
  ...
  PROCEDURE get_movies_helper(search_key IN OUT VARCHAR2, key_type IN VARCHAR2, movie_list OUT SYS_REFCURSOR) AS
  ...
  END get_movies_helper;
  
  PROCEDURE get_movies (search_key IN VARCHAR2, key_type IN VARCHAR2, c_movie_list OUT SYS_REFCURSOR) AS
      l_search_key VARCHAR2(100):= search_key;
    BEGIN
      get_movies_helper( search_key => l_search_key, key_type => key_type, movie_list => c_movie_list);
    END get_movies;
...
DECLARE
  ...
  --l_search_key VARCHAR2(100):= '1';
  --l_search_key VARCHAR2(100):= 'PG-13';
  --l_search_key VARCHAR2(100):= '04-MAR-16';
  --l_search_key VARCHAR2(100):= '1972';
  --l_search_key VARCHAR2(100):= 'god father';
